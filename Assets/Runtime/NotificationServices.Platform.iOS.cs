        ...
        private void SendiOSNotification(NotificationData data)
        {
            try
            {
                ...
                if (data.repeats && data.repeatInterval != RepeatInterval.None)
                    ...
                    {
                        TimeInterval = new TimeSpan(0, 0, data.fireTimeInSeconds), Repeats = false
                    };
                ...
                if (data.customBadgeCount >= 0)
                {
                    ...
                    if (!autoIncrementBadge) currentBadgeCount = data.customBadgeCount; // Sync with explicit count
                }
